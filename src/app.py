...
import os
import joblib
# On réutilise le module de l'étape précédente pour valider les critères du prof !
from database import get_db_connection

app = Flask(__name__)

# Définition des chemins des artefacts du modèle
...
# Chargement global des modèles au démarrage de l'API (Gain de performance)
try:
    ...
    print(f"⚠️ Attention : Impossible de charger les modèles ({e}). Lancez d'abord src/model.py")
...
def analyze_tweets():
    ...
    if not request.is_json:
        return jsonify({"error": "Le format de la requête doit être strictement du JSON."}), 400
    ...
    if tweets is None or not isinstance(tweets, list):
        return jsonify({"error": "Format incorrect. Un tableau de chaînes de caractères (string[]) est attendu."}), 400
    
    if len(tweets) == 0:
        return jsonify({"error": "La liste de tweets fournie est vide."}), 400
    
    if not all(isinstance(tweet, str) for tweet in tweets):
        return jsonify({"error": "Tous les éléments du tableau doivent être des chaînes de caractères."}), 400
    
    if vectorizer is None or model_positive is None or model_negative is None:
        return jsonify({"error": "Les modèles ne sont pas initialisés sur le serveur."}), 500
...
if __name__ == '__main__':
    # On écoute sur 0.0.0.0 pour permettre les tests externes ou la conteneurisation future
